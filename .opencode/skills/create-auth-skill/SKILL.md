...
â”œâ”€ YES â†’ New project setup
â”‚   1. Identify framework
â”‚   2. Choose database
â”‚   3. Install better-auth
â”‚   4. Create auth.ts + auth-client.ts
â”‚   5. Set up route handler
â”‚   6. Run CLI migrate/generate
â”‚   7. Add features via plugins
â”‚
â””â”€ NO â†’ Does project have existing auth?
    â”œâ”€ YES â†’ Migration/enhancement
    â”‚   â€¢ Audit current auth for gaps
    â”‚   â€¢ Plan incremental migration
    â”‚   â€¢ See migration guides in docs
    â”‚
    â””â”€ NO â†’ Add auth to existing project
...
|-----------|------|---------|
| Next.js App Router | `app/api/auth/[...all]/route.ts` | `toNextJsHandler(auth)` â†’ export `{ GET, POST }` |
| Next.js Pages | `pages/api/auth/[...all].ts` | `toNextJsHandler(auth)` â†’ default export |
