...
export async function stockRoutes(app: FastifyInstance) {
  app.get('/api/stock-items', async (request) => {
  ...
  app.post('/api/stock-items', { preHandler: [authenticate, authorize(['DEV_MASTER', 'ADMIN'])] }, async (request, reply) => {
  ...
  app.put('/api/stock-items/:id', { preHandler: [authenticate, authorize(['DEV_MASTER', 'ADMIN'])] }, async (request, reply) => {
  ...
  app.delete('/api/stock-items/:id', { preHandler: [authenticate, authorize(['DEV_MASTER'])] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    await db.delete(stockItems).where(eq(stockItems.id, id));
    return reply.code(204).send();
  });

  app.patch('/api/stock-items/:id/machines', { preHandler: [authenticate, authorize(['DEV_MASTER', 'ADMIN'])] }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  app.post('/api/stock-transactions', { preHandler: [authenticate] }, async (request, reply) => {
  ...
  app.get('/api/stock-transactions', { preHandler: [authenticate] }, async () => {
