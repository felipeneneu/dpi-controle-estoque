...
| BR-024 | imposition | **Sobra de grade no impositor**: quando o alvo de cópias não preenche as linhas exatas, o CLI **preenche a última linha** (fill_row → sobra "para refile/amostra") por padrão; `--surplus truncate` corta exato e `--surplus fill_advance` desloca o grid para maximizar aproveitamento. A contagem desenhada no PDF **sempre** coincide com a reportada (Regra 4 — jamais truncar silenciosamente). | IMPLEMENTED | `sidecars/AutoImposerCLI/Imposition/ImpositionBridge.cs:96` (default FillRow); `sidecars/AutoImposerCLI/Program.cs` (ParseSurplus) | — | BR-010 (testes BR-010_ag/ah/ai) |
