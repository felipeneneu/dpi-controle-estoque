...
| BR-002 | inventory/vision | Cada **bobina física é um ativo rastreável individual**: id por rolo, metros restantes, estado (em uso / em estoque / vazia / bloqueada / sucata) e localização. As somas por SKU passam a ser **derivadas** (Σ das bobinas). | IMPLEMENTED | `schema.ts:98`; `backend/src/routes/stock.ts:133-149` | **ADR-009**, **ADR-013** | BR-001, BR-018, BR-019 |
